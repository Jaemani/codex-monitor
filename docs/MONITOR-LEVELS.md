...
| 2. Conversation-scoped management | Create multiple named monitors for the current conversation; list, inspect, pause, resume and remove them without affecting another conversation. | Implemented for local files; scoped process checks and actual ordinary TUI PASS. Desktop managed-event consumption pending. |
| 3. Persistent observation and recovery | Run collectors under an owned service lifecycle, report failures, restore checkpoints after restart and distinguish configuration from observed health. | Implemented for a managed local file collector; process crash/checkpoint recovery, real macOS LaunchAgent lifecycle and 60-second soak PASS. Broader liveness monitoring remains future work. |
| 4. Conditional reactions | Debounce/coalesce relevant changes, sustained-condition thresholds, expiry and explicit escalation policies outside the model. | Basic unchanged-state suppression exists; general condition policies remain future work. |
