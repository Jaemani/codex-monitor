...
| 4. Conditional reactions | Debounce/coalesce relevant changes, sustained-condition thresholds, expiry and explicit escalation policies outside the model. | Stable-sample debounce and bounded JSON predicates passed deterministic and installed process checks. A 300-second predicate soak and exact-ID matched/recovery events in an ordinary Unix remote TUI passed. Escalation policies remain future work. |
| 5. Correlated two-way work | Track external requests through dispatch, worker acknowledgement, completion and explicit replies. | Durable request state and ordered notifications passed installed macOS/Linux checks and an actual ordinary TUI flow. State reports and source-scoped replies are explicit; delivery never implies completed work. Archival and messenger-specific workflows remain future work. |
